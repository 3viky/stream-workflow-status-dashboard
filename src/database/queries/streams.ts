...
export function insertStream(db: Database.Database, stream: Stream): void {
  const stmt = db.prepare(`
    INSERT INTO streams (
      ...
      updated_at, completed_at, last_file_change, phases
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
/**
 * Update stream's last file change timestamp
 */
export function updateLastFileChange(db: Database.Database, streamId: string, timestamp: string): void {
  const stmt = db.prepare(`
    UPDATE streams SET last_file_change = ?, updated_at = ? WHERE id = ?
  `);

  stmt.run(timestamp, new Date().toISOString(), streamId);
}
...
function rowToStream(row: Record<string, unknown>): Stream {
  return {
    ...
    lastFileChange: (row.last_file_change as string) ?? undefined,
